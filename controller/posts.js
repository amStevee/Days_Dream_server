...
const getPost = async (req, res) => {
  ...
  const q =
    'SELECT posts.id, username, title, description, users.user_image, posts.image, category, posts.created_at FROM posts JOIN users ON posts.userid = users.userid WHERE posts.id = $1 ORDER BY posts.id DESC';
